...
const EditBook = () => {
    ...
    const { book, fetchBook, editBook } = useBook();
    const [editableBook, setBook] = useState<Partial<Book>>({
        title: '',
        description: '',
        ...book
    });
    ...
    const handleEditBook = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (bookId && auth && role === Role.author && editableBook.title && editableBook.description && username === editableBook.author) {
            await editBook(bookId.toString(), editableBook.title, editableBook.description);
        }
    };
    ...
    return (
        <>
            <Head>
                <title>Edit - {book?.title || "Unknown"}{config.titleWithSeparator}</title>
            ...
            <div className="w-full flex justify-center mt-20">
                <div className="w-full max-w-5xl relative overflow-x-auto shadow-md p-4 lg:p-8">
                    <form className="w-full" onSubmit={handleEditBook}>
                        <div className="mb-6">
                        ...
                        <button className="w-full text-white bg-black hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg font-semibold text-sm px-4 py-2 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save Changes</button>
                    </form>
                </div>
            </div>
        </>
